...
begin
  if new.level > old.level then
    ...
    values (new.user_id, new.level, old.level, now())
    on conflict (user_id, level) do update set
      previous_level = least(public.level_up_celebrations.previous_level, excluded.previous_level),
      queued_at = least(public.level_up_celebrations.queued_at, excluded.queued_at);
  end if;
  return new;
end;
$$;
